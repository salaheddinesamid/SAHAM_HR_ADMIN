  ...
  const handleProfessionalDetailsChange = (e) =>{
    ...
    useEffect(()=>{
      setRequestDto(
        ...
        employeeBalance: {
            year: new Date().getFullYear(),
            annualBalance: employee?.balanceDetails?.annualBalance,
            accumulatedBalance: employee?.balanceDetails?.accumulatedBalance,
            usedBalance: employee?.balanceDetails?.usedBalance,
            remainderBalance : employee?.balanceDetails?.remainderBalance,
            previousYearBalance : employee?.balanceDetails?.previousYearBalance
        }
        }
      )
    },[employee])
        ...
        <Stack spacing={3} mt={1}>
          ...
          <Paper elevation={0} sx={{ p: 3, borderRadius: 2 }}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={3}>
                <TextField label={`Solde ${new Date().getFullYear() - 1}`} type="number" fullWidth name="previousYearBalance" value={requestDto.employeeBalance.previousYearBalance} aria-readonly/>
              </Grid>
